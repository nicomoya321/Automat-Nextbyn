...
def tickets_para_manana():
    ...
    # Validar columnas
    for col in [columna_propietario, columna_fecha_vencimiento, columna_ticket, columna_estado, columna_mesa]:
        if col not in df.columns:
            raise ValueError(f"La columna '{col}' no existe. Columnas detectadas: {df.columns.tolist()}")

    # 🔹 CONVERSIÓN SEGURA DE FECHA SIN IMPORTAR FORMATO EXACTO
    df[columna_fecha_vencimiento] = pd.to_datetime(df[columna_fecha_vencimiento], errors="coerce")

    # Limpiar ticket (quitar .0)
    df[columna_ticket] = pd.to_numeric(df[columna_ticket], errors="coerce").fillna(0).astype(int).astype(str)
    ...
    # Fecha de mañana automática
    manana = (datetime.now() + timedelta(days=1)).date()

    # Filtrar tickets que vencen mañana solo por día/mes/año
    df_manana = df[df[columna_fecha_vencimiento].dt.date == manana]
